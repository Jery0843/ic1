...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Validate file size (max 20MB for papers)
    ...
    if (user.paper_status === 'submitted') {
      return NextResponse.json(
    ...
    const fileName = `paper_${email}_${Date.now()}_${file.name}`;
    ...
    try {
      ...
      const messageData = {
        ...
        subject: `Paper Submission - ${user.name}`,
        text: `New paper submission received:
...
Submitted at: ${new Date().toLocaleString()}`,
        html: `<h2>New Paper Submission</h2>
        ...
        <p>Please find the paper attached.</p>`,
        attachment: {
          data: fileStream,
    ...
    await db.updateUser(email, {
      ...
      paper_submitted_at: new Date().toISOString(),
    } as any);
